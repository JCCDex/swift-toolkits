...
2. **did-bridge.js 网关注入**：迁移时把 `IpfsClient` 的 baseURL 占位化（见 03 章），同时与 Kotlin `:webview-bridge` 的 `did-bridge.js` 做 `diff` 对齐，避免方法集漂移；占位化造成的一行差异需在 CI diff 中白名单放行（或 Kotlin 侧同步占位化）。
...
6. **M-15 结构校验**：`signCredentialForDApp` 校验 `credential` 必须含 `@context|type`、`credentialSubject`、`issuer|issuerObject`（三条，对齐 Kotlin M-15）；不要放大为内容校验（用户确认在宿主 UI）。
...
14. **`didStat` 失败即发布失败（有限重试）**：previousCid 取不到时**先有限重试（1–2 次）再中止发布并上抛**（Kotlin 静默吞错导致 IPFS 链分叉，见 01 §6），不得静默继续。
...
19. **`signCredentialForDApp` 补 keyDoc 校验**：M-15 三条之外校验 `keyDoc.did`/`keyDoc.id`（JS 强依赖，见 03 §2），先于桥报错返回 `SwiftDidError.invalidCredential`。
...
22. **网关注入临时 bundle 缓存（方案 A）**：`makeTempBundle` 按「整个资产集的内容 hash」缓存目录（did-bridge.js / did-bridge.html / did-0.3.2.min.js / jcc-wallet-4.0.8.min.js 一并 hash；替换后的 did-bridge.js 已内嵌 ipfsBaseURL，hash 已覆盖网关，无需再拼 URL；目录如 `Application Support/Did/bridge-<hash>/`）——仅 hash 单个 did-bridge.js 会在其余资产升级时命中旧缓存。`start()` 幂等复用，避免每次重建/复制 4 个资产文件；`destroy()` 不删缓存目录（下次启动复用）；启动时清理与当前 hash 不匹配的旧 `bridge-*` 目录，避免网关变化后累积。
...
| 桥单测 | `FakeDidBridge`（对齐 Kotlin `installBridgeForTest`） | 参数构造、返回解析、`signCredentialForDApp` 结构校验分支（缺 @context/credentialSubject/issuer、**缺 keyDoc.did/id**） |
...
| 真实桥冒烟（iOS） | 复用 SwiftWebviewBridge 集成基建 | `didResolve` / `generatePublicKeyBase58` 端到端（含网关注入后 publish 路径） |
...
- [ ] 与 Kotlin `:webview-bridge` 对齐 `did-bridge.html` / `did-bridge.js`（`diff` 校验，占位行白名单放行）
- [ ] `did-bridge.js` IPFS 网关占位化 + 注入机制（临时 bundle / user script）+ URL 校验（修复 D5）
